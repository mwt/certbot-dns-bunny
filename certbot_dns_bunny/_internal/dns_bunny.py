...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    description = "Obtain certificates using a DNS TXT record (if you are using Bunny.net for DNS)."
    ttl = 120
    ...
    def more_info(self) -> str:
        return "This plugin configures a DNS TXT record to respond to a dns-01 challenge using the Bunny.net API."

    def _validate_credentials(self, credentials: CredentialsConfiguration) -> None:
        key = credentials.conf("api-key")
        if not key:
            raise errors.PluginError(
                "{}: api-key required for authentication (see {})".format(
    ...
    def _perform(self, domain: str, validation_name: str, validation: str) -> None:
        self._get_bunny_client().add_txt_record(
...
class _BunnyClient:
    ...
    def __init__(self, api_key: str) -> None:
        self.headers = {"Accept": "application/json", "AccessKey": api_key}
    ...
    def add_txt_record(
        ...
        response = requests.put(
            f"https://api.bunny.net/dnszone/{zone_id}/records",
            json=payload,
        ...
        if zone_id:
            ...
            if record_id:
                response = requests.delete(
                    f"https://api.bunny.net/dnszone/{zone_id}/records/{record_id}",
    ...
    def _find_txt_record_id(
        self, zone_id: str, record_name: str, record_content: str
    ) -> Optional[str]:
        """
        ...
        dnszone_request = requests.get(
            f"https://api.bunny.net/dnszone/{zone_id}", headers=self.headers
        )
